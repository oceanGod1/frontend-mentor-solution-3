...
const selectedElemennts = {
  productID: document.querySelector('h1').innerHTML.split(' ').join('-').toLocaleLowerCase(),
  productThumbnailIndex: Array.from(document.querySelectorAll(".thumbnail-cards figure")),
  itemsAddedSuccessMessage: document.querySelector(".item-added-success-message"),
...
const reusables = {
  ...
  cartBadgeState: function () {
    ...
    cartBadge.innerHTML != 0 ? cartBadge.classList.remove("hidden") : cartBadge.classList.add("hidden")
  },
  ...
  scrollWithThumbnail: function (elem, arr, reusableMethod, imageHolder, num, previous, next){
    elem.addEventListener('click', () => {
      ...
      arr.indexOf(elem) === 0 ? previous.disabled = true : previous.disabled = false;
      arr.indexOf(elem) === arr.length - 1 ? next.disabled = true : next.disabled = false;
    })
  },
  ...
  newCartItem: () => {
    ...
    cartItems[`${productID}`].totalCost = document.querySelector(".product-price span").innerHTML * cartItems[`${productID}`].quantity;
    cartContent.insertAdjacentHTML("beforeend",
      ...
      <div class="${productID} grid grid-cols-[auto_1fr_auto] grid-rows-[2rem_2rem] gap-x-4">
        ...
        <div class="col-start-2 flex items-center">
          <span class="calculate-price text-secondaryLight self-center">${document.querySelector(".product-price").innerHTML} x ${cartItems[`${productID}`].quantity}</span>
          <span class="total-price-per-item pl-4 text-secondary font-bold">$${cartItems[`${productID}`].totalCost.toPrecision(String(cartItems[`${productID}`].totalCost).length + 2)}</span>
        </div>
      </div>
      `
    );
...
const applyScrollButtonsUI = (() => {
  ...
  reusables.applyScrollButtonsUI(gotoNextImage, gotoPreviousImage, () =>{
    ...
    const {backdrop} = selectedElemennts
    const clonedGotoNextImage = backdrop.firstElementChild.children[0].children[2];
    return clonedGotoNextImage
  }, () => {
    const {backdrop} = selectedElemennts
    const clonedGotoPreviousImage = backdrop.firstElementChild.children[0].children[1];
...
const applyScrollWithGoToButtons = (() => {
  const {gotoNextImage, gotoPreviousImage, productImageContainer, productThumbnailIndex} = selectedElemennts;
...
const applyScrollWithThumbnails = (() => {
  const {gotoNextImage, gotoPreviousImage, productThumbnailIndex, productImageContainer} = selectedElemennts
  productThumbnailIndex.forEach(thumbnail => {
    reusables.scrollWithThumbnail(thumbnail, productThumbnailIndex, 'productImageGen', productImageContainer, 2, gotoPreviousImage, gotoNextImage);
  });
})();
...
activateImageClone.addEventListener('click', () => {
  ...
  clonedProductThumbnailIndex.forEach(thumbnail => {
    reusables.scrollWithThumbnail(thumbnail, clonedProductThumbnailIndex, 'fullScreenModeCurrentProductImage', clonedProductImageContainer, 0, clonedGotoPreviousImage, clonedGotoNextImage);    
    thumbnail.addEventListener('click', () => {
      reusables.productImageGen('left-0');
      productImageContainer.children[2].remove();
      reusables.highlightThumbnail(productThumbnailIndex, () => productThumbnailIndex[currentProductImageIndex - 1]);
      clonedProductThumbnailIndex.indexOf(thumbnail) === 0 ? gotoPreviousImage.disabled = true : gotoPreviousImage.disabled = false;
      clonedProductThumbnailIndex.indexOf(thumbnail) === clonedProductThumbnailIndex.length - 1 ? gotoNextImage.disabled = true : gotoNextImage.disabled = false;
    })
  });
});
...
const addToCartButtonOnClick = (() =>{
  const {productQuantity, addToCart, cartContent, productID} = selectedElemennts;
  addToCart.addEventListener("submit", (e) => {
    ...
    if (productQuantity.value >= 1 && !cartContent.classList.contains("not-empty")) {
      ...
      cartContent.insertAdjacentHTML("afterend", `<button class="checkout-button h-16 rounded-xl text-secondary font-bold bg-primary col-start-2">Checkout</button>`)
      cartContent.classList.add("not-empty");    
    } else if (productQuantity.value >= 1 && cartContent.classList.contains("not-empty") && !cartItems[`${productID}`]) {
      reusables.newCartItem();
      reusables.cartBadgeState();
      reusables.successMessage();
    } else if (productQuantity.value >= 1 && cartContent.classList.contains("not-empty") && cartItems[`${productID}`]) {
      cartItems[`${productID}`].quantity += productQuantityIndex;
      cartItems[`${productID}`].totalCost = document.querySelector(".product-price span").innerHTML * cartItems[`${productID}`].quantity;
      document.querySelector('.calculate-price').innerHTML = `${document.querySelector(".product-price").innerHTML} x ${cartItems[`${productID}`].quantity}`
      document.querySelector('.total-price-per-item').innerHTML = `$${cartItems[`${productID}`].totalCost.toPrecision(String(cartItems[`${productID}`].totalCost).length + 2)}`
      reusables.cartBadgeState();
      reusables.successMessage();
    }
